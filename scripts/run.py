...
#(query_elliptical_pixels,{'nside':nside, 'ra':335.0,'dec':17.5,'major':67,'minor':67,'pa':0,'res':12})
    ...
    # regions.add_region(Region(name='North Polar Spur', cmap='Reds',nside=nside,
    #                             processes = [(query_elliptical_pixels,{'nside':nside, 'ra':335.0,'dec':17.5,'major':67,'minor':67,'pa':0,'res':12}),
    #                                         (subtract_elliptical_pixels,{'nside':nside, 'ra':329.0,'dec':17.5,'major':40,'minor':40,'pa':0,'res':12}), # Inner North Polar Spur
    #                                         (subtract_elliptical_pixels,{'nside':nside, 'ra':0,'dec':0,'major':13,'minor':60,'pa':0,'res':18}), # Inner Plane
    #                                         (subtract_elliptical_pixels,{'nside':nside, 'ra':6,'dec':24,'major':5,'minor':5,'pa':0,'res':18}), # Sh2-27
    #                                         (latitude_cut,{'latitude':0,'model':'<'}),
    #                                         (bad_data_mask,{'map':hp.ud_grade(cbass_map,nside)})]))
...
def plot_masks():
    ...
    #regions.create_mask('masks/CBASS_95pc.fits',include=['CBASS 95pc'])
    #regions.create_mask('masks/CBASS_90pc.fits',include=['CBASS 90pc'])
    #regions.create_mask('masks/CBASS_85pc.fits',include=['CBASS 85pc'])
    #regions.create_mask('masks/CBASS_80pc.fits',include=['CBASS 80pc'])
    #regions.create_mask('masks/Fan_Region.fits',include=['Fan Region'])
    regions.create_mask('masks/NorthPolarSpur.fits',include=['North Polar Spur'],exclude=['LFI30 PCCS'])
    regions.create_mask('masks/NorthPolarSpurA.fits',include=['North Polar Spur A'],exclude=['LFI30 PCCS'])
    regions.create_mask('masks/NorthPolarSpurB.fits',include=['North Polar Spur B'],exclude=['LFI30 PCCS'])
